...
import { isVoiceLanguageCode, resolveEffectiveLanguageCode } from '@/lib/voice/languages'
...
export async function POST(request: Request) {
  ...
  const language = resolveEffectiveLanguageCode(requestedLanguage ?? 'auto', transcript)

  try {
    const result = await handleVoiceCommand(transcript, history, {
      ...resolveAgentWorkspaceOptions(request),
      language,
    })
    return apiSuccess({ ...result, detectedLanguage: language })
  } catch (err) {
